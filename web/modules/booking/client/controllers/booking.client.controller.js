(function () {
  ...
  function bookingController($scope, Authentication, $rootScope, $http, HotelsServices, patterns,
    ...
    // Params - Room details, Guest details, Promo details
    ...
    function calculateAll(room, guests, promo) {
      ...
      //multipliying by no of nights and no of rooms

      $scope.finalTaxs   = (room.price.tax * guests.rooms);
      $scope.subTotal    = (room.price.discounted * guests.rooms);
      $scope.totalAmount = ($scope.subTotal + $scope.finalTaxs);
      if ($scope.discounts > 0 && $scope.discounts > $scope.totalAmount) {
        $scope.totalAmount = 0;
      }
      if ($scope.discounts > 0 && $scope.discounts <= $scope.totalAmount) {
        $scope.totalAmount = $scope.totalAmount - $scope.discounts;
      }
    };
      ...
      $timeout(function () {
        $http.post('/api/validatePromo', $scope.promoInfo).success(function (res) {
          ...
          if (res && res.Result) {
            ...
            var promo           = (res.Result["Discount Price"] != undefined) ? res.Result["Discount Price"] : '0';
    ...
    $scope.book    = function (isValid) {
      ...
      $scope.body.serviceTax       = (($rootScope.currency == undefined) ? $scope.finalTaxs : ($scope.finalTaxs * $scope.currencyMultiplier[$scope.hotel.currency]));
      $scope.body.discounts        = (($rootScope.currency == undefined) ? $scope.discounts : ($scope.discounts * $scope.currencyMultiplier[$scope.hotel.currency]));
      ...
      _($scope.body.rooms).forEach(function (room) {
        ...
        room.amount = ($scope.body.subTotal + $scope.body.serviceTax) / ($scope.guest.rooms * $scope.guest.nights);
      });
      if ($scope.ipDetails != undefined) {
    ...
    $scope.bookDirect = function (firstName, lastName, emailId, mobileNo, address, spl_requests) {
      ...
      $scope.body.discounts        = (($rootScope.currency == undefined) ? $scope.discounts : ($scope.discounts * $scope.currencyMultiplier[$scope.hotel.currency]));
      $scope.body.promocode        = $scope.promoApplied ? $scope.promo : '';
      $scope.body.appliedPromoCode = $scope.discounts;
      ...
      if ($scope.body.ipDetails != undefined) {
        $scope.body.ipAddress = $scope.ipDetails.geobytesremoteip;
        $scope.body.ipDetails = $scope.ipDetails.geobytescountry + " - " + $scope.ipDetails.geobytesregion + " - " + $scope.ipDetails.geobytescity +
          " - Lat:" + $scope.ipDetails.geobyteslatitude + " - Long:" + $scope.ipDetails.geobyteslongitude + " - Currency:" + $scope.ipDetails.geobytescurrencycode;
      }
